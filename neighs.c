...
int kf_would_interact(LR_system *syst, PatchyParticle *p, vector r, vector *patches, int *onp, int *onq) {
	...
	if(dist2 < 1.) return -1;
	else if(dist2 < syst->kf_sqr_rcut_aa) {
		// versor
...
int kf_interact(LR_system *syst, PatchyParticle *p, PatchyParticle *q, int *onp, int *onq) {
